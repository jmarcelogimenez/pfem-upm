...
The starting point is the problem documented by Guermond. The computational domain is $[-d/2,d/2]\times[-2d,2d]$ and the initial position of the perturbed interface is $\eta(x) = 0.1d \cos(2\pi x/d)$. The density ratio is $3$, which corresponds to an Atwood
...
Another relevant feature to take into account is that similar CPU times are required to solve a time-step when both algorithms are compared.
Table \ref{tb:times-rt} summarizes the CPU Times required to complete $1[s]$ of real time in the current case. Results show that using the same time-step both solvers have similar performance, being \OF faster. However, due to capability of time-step enlargement of PFEM, it achieves shorter CPU times with similar accuracy.
