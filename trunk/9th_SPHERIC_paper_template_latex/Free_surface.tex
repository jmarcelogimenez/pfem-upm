The accurate and efficient simulation of interface evolution is of fundamental importance in the simulation of multiphase flows. Large jumps of fluid density and viscosity across the interface should be correctly assumed by the numerical algorithm in order to satisfy the momentum balance and mass conservation at the vicinity of the interface.
...
where the nodes included in the interpolation are the nodes of the hosting element. Two situations could happen when a particle changes its position: either all the nodes of the hosting element can have the same density as the fluid particle or one or more nodes can have different density as the fluid particle. While in the first case, a typical finite element interpolation is performed, the second situation clearly represents a situation where the fluid particle is close to the interface. Let us focus on  this second possibility when there is high enough density ratio $\rho_1/\rho_2>\alpha$, in this case two situations can appear:
 ...
 where $\alpha$ and $\beta$ are numerical parameters. For those cases where density ratio is not excessive $\rho_1/\rho_2<\alpha$, equation (\ref{Interpolation}) is always used.
...
Once the particle have been advected and the intermediate velocities have been determined, mesh nodes have to incorporate this updated velocity, viscosity and density information. During this \textit{Projection Stage}, each node $j$ updates its intermediate velocity according to equation (\ref{Step3a}) and similarly its value $\lambda_j$. Depending on the value of $\lambda_j$, the node will be assigned to the first or the second fluid and the values of density and viscosity will be the corresponding ones. The instantaneous local interface inside each element is determined as the iso-line (an iso-plane in 3D) where $\lambda(\xx)=0$.

\subsection{Shape function enrichments for pressure gradient discontinuity capturing}

In typical finite element methods, the gradient of the shape functions $\nabla\phi_j$ are continuous within each element, and therefore any unknown interpolated is also continuous. When the interface crosses an element the discontinuity in the material properties leads to discontinuities in the gradients of the unknowns that a typical interpolation based on continuous gradient shape functions cannot capture. This practice for the case of two different density fluids produce interpolation errors in the pressure that give rise to spurious velocities which finally can render the solution meaningless.

The strategy followed by enrichment methods is increasing the number of degrees of freedom at elements cut by the interface in order to reduce interpolation errors. In this work, the enriched space mainly used is based on the one presented by Coppola\cite{Coppola05}, see Figure \ref{fg:enrichment1} as a illustrating example of a 2D triangular element with nodes 1, 2 and 3 crossed by the interface at points A and B. The line that joins points A and B divides the element into two sub-domains $\Omega_1$ and $\Omega_2$. The new degree of freedom could be statically condensed within each element in the pressure equation and then recovered in the correction step. Briefly, the way to construct the enrichment function $\phi^*$ is ensuring that
