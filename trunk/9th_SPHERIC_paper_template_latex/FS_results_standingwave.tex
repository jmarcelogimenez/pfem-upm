...
Regarding to computational effort, it is noticeable that the set of PFEM simulations have spent only few hours in an Intel i7-2600k 3.4GHz processor. This is a big difference with the CPU times reported on other works. As an example, in \cite{Colagrossi12} it is reported that, to solve a similar set of problems with SPH and to obtain similar accuracy than those presented in this work, they had to use a 30 CPUs cluster, each CPU with 8 Intel Xeon 2.33GHz cores, running continuously for 30 days.