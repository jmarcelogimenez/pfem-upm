...
After a complete mesh converge process based on the drag and lift forces, a final mesh has been selected. The mesh used for the simulations is a linear triangular mesh that contains 65834 triangles and 132542 nodes, the default mesh size is 0.655D while in the cylinder proximities and the free surface area the mesh size has been refined to 0.0016D. See figure \ref{f:geometryandmesh} to appreciate the computational domain and mesh configuration.
...
In order to confirm if the averaged baseflows obtained for highest supercritical Reynolds values were correct, a DMD analysis has been performed for each cylinder depth. In figure \ref{f:freqDMD}, the critical frequencies from the most unstable mode DMD analysis are compared to the ones coming from the global analysis. As can be observed the values of the frequencies agree and the leading frequency was in all cases attributed to the vortex shedding phenomena aft the cylinder.

\begin{figure}
  \begin{center}
  \includegraphics[width=\columnwidth]{Freq_h.pdf}
  \end{center}
...
The mode corresponding to this oscillating unstable mode for the case $Re_D=45$ $Fr_D=3$ and $h/D=0.55$ is shown in figure \ref{f:mode}, where the shape of the mode is visibly influenced by the free surface modulation.
