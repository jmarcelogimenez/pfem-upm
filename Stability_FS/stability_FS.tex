...
Here $\mathbf{v}$, $p$ are the velocity and fluid pressure and $\mathbf{u_g}$ is a unitary vector parallel to the gravity force. The equations are computed in non-dimensional form using the cylinder diameter $D$, the inflow velocity $U$ and the density and viscosity of the bottom fluid. The Reynolds $Re_D=\frac{UD}{\nu_{2}}$ and the Froude numbers $Fr_D=\frac{U}{\sqrt{gD}}$ will be defined using the fluid properties of the bottom fluid. It is important to remark that compared to the case where only one fluid is involved, two non dimensional coefficients $\alpha$ and $\beta$ ( both specially relevant when including the free surface interface) appear explicitly in the mass and momentum conservation equations \ref{eq:NS1} and \ref{eq:NS2}.
...
After a complete mesh converge process based on the drag and lift forces, a final mesh has been selected. The mesh used for the simulations is a linear triangular mesh that contains 65834 triangles and 132542 nodes, the default mesh size is 0.655D while in the cylinder proximities and the free surface area the mesh size has been refined to 0.0016D. See FIG. \ref{f:geometryandmesh} to appreciate the computational domain and the mesh configuration near the cylinder.
...
An accurate and efficient simulation of the interface evolution is crucial when simulating free-surface flows. During the flow evolution, it is essential that the interface remains sharp. Large jumps of fluid density and viscosity across the interface should be correctly captured by the numerical algorithm in order to satisfy the momentum balance at the vicinity of the interface. Readers interested in more details of the PFEM-2 method and the enrichment technique used for the free surface definition may see \cite{Gimenez2015186}.
...
where the complex number $\omega=\omega_r+i\omega_i$ contains the growth rate as real part and the oscillation frequency as complex part. The stability analysis of the equations implies the linearization of the Navier-Stokes equations around a steady or mean base flow. This process has been done following the same methodology explained in \cite{Gonzalez07} which implies the resolution of a large generalized eigenvalue problem by an iterative Arnoldi method. The baseflow can be considered unstable when any of the $\omega_r$ are positive. When the Navier-Stokes equations are solved and perturbed, new terms coming from the $\alpha$ and $\beta$ gradients are important and must be specially taken into account on free surface proximities. However, two important considerations should be remarked when the analysis step is performed:

\begin{itemize}
 \item For subcritical Reynolds numbers, the steady baseflow implies a final distribution of both fluids determined by the values of the VOF function. As usual, the free surface is represented by VOF function isosurface $\phi=0$. The VOF function distribution is not perturbed for the analysis, assuming that the vortex shedding will be the cause of instability. This will be later confirmed by DMD analysis.
...
In the future, these results will be also extended to higher Reynolds values by DMD analysis. The possibility of having situations where growing instabilities are caused by the free surface oscillations, similar to what occurs in a Rayleigh-Taylor instability, will be also analyzed. This work was supported by the Technical University of Madrid by the project (PID) "Combination of Eulerian and Lagrangian methodologies for the solution of free surface flows."
