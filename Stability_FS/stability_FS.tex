...
where $\varepsilon\ll 1$. The stability analyses of the equations implies the linearization of the Navier-Stokes equations around a steady base flow. This process has been done following the same methods explained in \cite{GonzalezTheofilisGomezBlanco} which implies the resolution of a large generalized eigenvalue problem by an iterative Arnoldi method. However, two important considerations should be remarked when this analysis step is performed:
...
According to Dimas results when the Froude number is above 2.5 as in our case the Branch II becomes dominant and instability becomes of the absolute type.\cite{dimas}
